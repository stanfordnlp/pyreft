...
class VLBartVideo(VLBart):
    ...
    def test_step(self, batch, **kwargs):
        ...
        if self.config.classifier:
            ...
            logits = output['logits'].detach().cpu().view(B, -1, self.lm_head.out_features)[:, 1]
            ...
            confidence = torch.softmax(binary_logits, dim=1)[:, 0] # The probability of true
            confidence = confidence.view(B//5, 5)
            confidence = confidence / confidence.sum(dim=1, keepdim=True)
            
            prediction = torch.argmax(confidence, dim=1).numpy().tolist() # list of indices
