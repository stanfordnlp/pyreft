...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        self.model.intervenable.unfreeze_intervention_parameters() # unfreeze intervention parameters
        print(self.model)
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                # self.model.module.intervenable.train()
                # self.model.module.train()
                # self.model.module.zero_grad()
                # print("BEGIN EPOCH!")
                # for n, p in self.model.named_parameters():
                #     if p.grad is not None:
                #         print(n, p.grad.norm().item(), p.data.norm().item())
                #     else:
                #         print(n, "GRAD IS NONE", p.data.norm().item())
                # print("START GRAD")
                if self.args.fp16 and _use_native_amp:
                ...
                if self.args.fp16 and _use_native_amp:
                    ...
                    self.optim.step()
                # for elt in self.optim.param_groups[0]["params"]:
                #     print(elt.norm().item())
                # print("AFTER BACKPROP")
                # for n, p in self.model.named_parameters():
                #     if p.grad is not None:
                #         print(n, p.requires_grad, p.grad.norm().item(), p.data.norm().item())
